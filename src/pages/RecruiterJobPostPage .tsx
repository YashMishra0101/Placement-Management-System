...
//                   >
//                     {/* ... (rest of the applicant card remains the same) ... */}
//                   </motion.div>
//                 ))}
//               </AnimatePresence>
//             ) : (
...
//   const [activeTab, setActiveTab] = useState<"jobPosting" | "applicants">(
//     "jobPosting"
//   );
...
const ManageJobs = ({ jobs, onDelete }: { jobs: any[]; onDelete: (jobId: string) => void }) => {
...
const RecruiterJobPostPage = () => {
  const [activeTab, setActiveTab] = useState<"jobPosting" | "applicants" | "manageJobs">(
    "jobPosting"
  );
  const [students, setStudents] = useState<any[]>([]);
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(db, "jobPosts"),
      where("recruiterId", "==", user.uid) // Ensures only this recruiter's jobs
    );
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      const jobs = querySnapshot.docs.map((doc) => ({
        ...
        jobTitle: doc.data().jobTitle || "Untitled Position",
